  ...
  <div class="user-content">
    <i-form inline :label-width="80">
      <i-row>
        ...
        <i-form-item prop="userCode" inline label="工号">
          <i-input type="text" v-model="condition.userCode" placeholder="请输入检索工号">
          </i-input>
        </i-form-item>

        <i-form-item prop="dept" label="科室">
          <i-select v-model="condition.deptCode" style="width:160px" placeholder="请选择待筛选科室">
            <i-option v-for="item in deptList" :value="item.deptCode" :key="item.deptCode">{{ item.deptName }}</i-option>
          </i-select>
        </i-form-item>
      ...
      <i-row>
        <i-form-item prop="role" label="角色">
          <i-select v-model="condition.roleId"  style="width:160px" placeholder="请选择待筛选角色">
        ...
        <i-form-item prop="startDate" inline label="入职时间">
    ...
    <div>
      <i-table border stripe :columns="userInfoColumns" :data="userInfoArray" class="user_grid"></i-table>
    </div>
        ...
        <i-row>
          <i-col span="12">
            ...
            <i-form-item label="用户名" prop="username">
              <i-input placeholder="请输入用户名" v-model="editFormItem.username" style="width:160px"></i-input>
            </i-form-item>

            <i-form-item  label="工号" prop="userCode">
              <i-input v-model="editFormItem.userCode" placeholder="请输入检索工号" style="width:160px">
              </i-input>
            </i-form-item>
            ...
            <i-form-item prop="state" label="状态">
              <i-select v-model="editFormItem.state" style="width:160px" placeholder="请选择待筛选状态">
                <i-option value="0">正常</i-option>
                <i-option value="1">冻结</i-option>
              </i-select>
            </i-form-item>
          ...
          <i-col span="12">
            ...
            <i-form-item  label="密码" prop="password" v-show="editFormItem.operatorType==='0'">
              <i-input v-model="editFormItem.password" disabled style="width:160px">
              </i-input>
            </i-form-item>

            <i-form-item label="科室" prop="deptCode">
              <i-select v-model="editFormItem.deptCode" style="width:160px" placeholder="请选择待筛选科室">
                <i-option v-for="item in deptList" :value="item.deptCode" :key="item.deptCode">{{ item.deptName }}</i-option>
              </i-select>
            </i-form-item>
           ...
           <i-form-item prop="entryTime" inline label="入职时间">
              <i-date-picker :value="editFormItem.entryTime" format="yyyy-MM-dd HH:mm:ss" @on-change="getEntryTime" type="datetime" placeholder="请选择开始时间" style="width: 160px"></i-date-picker>
            </i-form-item>
          </i-col>
        </i-row>
      </i-form>
    ...
    data () {
      return {
        ...
        ruleValidate: {
          ...
          userCode: [
            { required: true, message: 'The userCode cannot be empty', trigger: 'blur' }
          ],
          roleId: [
          ...
          deptCode: [
            { required: true, message: 'The deptCode cannot be empty', trigger: 'blur' }
          ]
        }
      }
    },
    ...
    methods: {
      ...
      getDeptAndRoleInfo () {
        ...
        let userInfo = JSON.parse(Cookies.get('userInfo'))
          this.$axios.ajax({
            method: 'get',
            url: '/user/getDeptAndRoleInfo?hosCode=' + hosCode,
            data: {}
          }).then((res) => {
            if (!!res.data.code && 10000 == res.data.code) {
              let data = res.data.data
              this.deptList = data['deptInfo']
